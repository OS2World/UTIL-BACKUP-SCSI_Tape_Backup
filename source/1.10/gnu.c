/*****************************************************************************
 * $Id: gnu.c,v 1.11 1993/01/10 11:19:45 ak Exp $
 *****************************************************************************
 * $Log: gnu.c,v $
 * Revision 1.11  1993/01/10  11:19:45  ak
 * J.R.: Bugfix: f_gnudump='f' sicherte doppelt.
 *
 * Revision 1.10  1992/12/13  09:39:40  ak
 * K.U.R: f_archive/f_reset_archive added to support the "archived" bit.
 *
 * Revision 1.9  1992/10/31  06:55:09  ak
 * Modified -G (again) to specify the generation of increment.
 *
 * Revision 1.8  1992/10/29  10:00:03  ak
 * -G for incremental backups like -g, but w/o writing the dumpfile.
 *
 * Revision 1.7  1992/09/29  09:55:42  ak
 * K.U.R., once again :-)
 * - removed a dup() in buffer.c
 * - EMX opendir with hidden/system test in gnu.c (...dotdot)
 *
 * Revision 1.6  1992/09/26  08:31:50  ak
 * *** empty log message ***
 *
 * Revision 1.5  1992/09/20  17:50:30  ak
 * EMX 0.8 stat() returns random inode numbers. As a result, TAR -g
 * archived every single directory. ifdef'ed out this code.
 *
 * Revision 1.4  1992/09/20  07:47:06  ak
 * Fixes from Kai Uwe Rommel
 *   --checkpoints instead of --semi-verbose (1.11)
 *   -g filenames
 *
 * Revision 1.3  1992/09/12  15:57:30  ak
 ...
 *
 * Revision 1.2  1992/09/02  20:08:10  ak
 * Version AK200
 * - Tape access
 * - Quick file access
 ...
 *
 * Revision 1.1.1.1  1992/09/02  19:21:41  ak
 * Original GNU Tar 1.10 with some filenames changed for FAT compatibility.
 *
 * Revision 1.1  1992/09/02  19:21:40  ak
 * Initial revision
 *
 *****************************************************************************/

static char *rcsid = "$Id: gnu.c,v 1.11 1993/01/10 11:19:45 ak Exp $";
...
read_dir_file()
{
	...
	time(&this_time);
#ifdef MSDOS
	if(gnu_dumpfile[0]!='/' && (!isalpha(gnu_dumpfile[0]) || 
				    gnu_dumpfile[1]!=':' || 
				    gnu_dumpfile[2]!='/')) {
		_fullpath(path,gnu_dumpfile,MAXPATHLEN);
		gnu_dumpfile=path;
	}
#else
	if(gnu_dumpfile[0]!='/') {
...
struct dirent *d;
{
  	char *p = d->d_name;
#ifdef MSDOS
        if(!f_use_protection && (d->d_mode & (A_HIDDEN|A_SYSTEM)) != 0)
	  	return 1;
        if(f_archive && (d->d_mode & (A_ARCHIVE|A_DIR)) == 0)
	  	return 1;
#endif
